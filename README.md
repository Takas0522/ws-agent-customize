...
アプリは `src/index.html` から `resources/custom-controls.min.*` を参照しているため、**プロジェクトルート** から HTTP サーバを起動する必要があります（`src/` 直下で起動するとパスが解決できません）。
...
> ⚠️ `http-server` のデフォルトキャッシュ (1 時間) が有効だと、`resources/*.min.js` を更新してもブラウザに反映されない場合があります。開発中は `-c-1`（キャッシュ無効）または DevTools の「Disable cache」をご利用ください。
...
- **`src/` 直下で `http-server` を起動しても動かない**: `../lib/...` への相対参照がサーバ外になります。プロジェクトルートで起動してください。
